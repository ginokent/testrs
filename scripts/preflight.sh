#!/usr/bin/env bash
# scripts/preflight.sh — `mise run ci` / `mise run ci-publish` の事前条件 (dirty
# check) を実行する。modified / staged の差分があれば fail。
#
...
error: uncommitted changes in tracked files.
       Commit or stash before running 'mise run ci' / 'mise run ci-publish'.
EOF
  git status --short >&2
  exit 1
fi
