...
1. **`#[derive(Arbitrary)]` のフィールド属性
   ...
   サポート。named-field 構造体・tuple 構造体・enum variant の
   フィールド (両形式) に適用可能。
...
7. 浮動小数点の近似等価 assert 用 **`prop_assert_close!`** マクロ。
...
- **`Fields::Named` / `Fields::Unnamed` 表現。** 双方とも従来の
  ...
  同じ仕組みで動くようになる。
- **`prop_recursive!` の型付け。** `inner` closure は
...
- カバレッジ駆動 fuzzing、regex 文字列、async ランタイム、スナップ
