...
Rust 向けの dev-dependency プロパティベーステスト + fuzzing
...
```
crates/
├── propcheck-core/   # Rng + Arbitrary trait + Strategy combinator
├── propcheck-derive/ # proc-macro: #[derive(Arbitrary)] + #[propcheck]
├── propcheck/        # ランナー、assertion マクロ、regression replay
└── propcheck-fuzz/   # in-process mutation・バイト fuzzer + 型付きファズ
```

umbrella クレート `propcheck` が `-core` と `-derive` の内容を
すべて再エクスポート。ユーザは通常 `propcheck` のみに依存
(fuzzing を使う場合は `propcheck-fuzz` も)。
...
- 型ごとの shrink: 整数は 0 方向 (halve-then-walk-back)、
  コレクションは長さ → 要素ごと、文字列は char vec 経由。
- ランナーの shrink ループは「まだ失敗する最初の候補を採用」
...
- `fuzz(cfg, |&[u8]| ...)` — ミューテートされたバイト列を
  ターゲットに流す。Crash 信号として panic を使用。初期 corpus は
  任意。
- mutation: bit-flip, byte-replace, interesting-byte,
  insert, delete, splice (corpus から), swap。
...
- `fail_demo` — 意図的失敗時の shrink 出力デモ。
- `find_crash` — バグを仕込んだ parser の fuzzing。

## 未対応 (この時点)

初期リリースでは意図的に以下を含めず:

- カスタム strategy / combinator (M2 で追加)。
- `#[derive(Arbitrary)]` (M3 で追加)。
- assertion マクロ、`prop_assume!`、classify、async、状態機械 —
  以降のマイルストーンで全部追加。
