...
pub fn pbt(attr: TokenStream, item: TokenStream) -> TokenStream {
...
fn parse_attr_args(attr: TokenStream) -> Result<AttrArgs, String> {
    ...
    while iter.peek().is_some() {
        ...
        match key.as_str() {
            ...
            other => return Err(format!("unknown #[pbt] argument: `{other}`")),
        }
        if matches!(iter.peek(), Some(TokenTree::Punct(p)) if p.as_char() == ',') {
            iter.next();
        }
...
fn parse_fn(input: TokenStream) -> Result<ParsedFn, String> {
    ...
    if matches!(iter.peek(), Some(TokenTree::Punct(p)) if p.as_char() == '<') {
        return Err("generic test functions are not supported by #[pbt]".to_string());
    }
