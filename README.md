...
現在は **PBT** (プロパティベーステスト) と **fuzzing** の 2 カテゴリを提供し、
共有基盤 `testrs-core` の上に PBT 系 (`testrs-pbt` / `testrs-pbt-derive`) と
fuzzing 系 (`testrs-fuzz`) が乗る。**fuzzing は PBT の一部ではなく別カテゴリ**
であり、両者は共有基盤を並行して利用する兄弟関係にある (`testrs-fuzz` は
`testrs-core` のみに依存し、PBT ランナー `testrs-pbt` には依存しない)。
PBT も fuzzing も testrs が持つ複数カテゴリの一つにすぎない。
...
のは `testrs-pbt` (PBT 利用) と `testrs-fuzz` (fuzzing 利用) の 2 つ。

今後 PBT / fuzzing とは異質なテストカテゴリを加える場合は、既存 crate の
feature として押し込まず、**新しい兄弟 crate として並べて追加する**。
テストカテゴリを crate 単位で分離するのが本リポジトリの拡張方針である。
